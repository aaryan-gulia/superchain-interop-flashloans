...
contract Deploy is Script {
    ...
    string[] private rpcUrls = [
        ...
        "https://interop-alpha-1.optimism.io/"
    ];
    ...
    function run() public {
        for (uint256 i = 0; i < rpcUrls.length; i++) {
            ...
            address uniswapDummyContractAddress = deployUniswapDummyContract(
                testUSDTokenAddress
            );
            address flashBorrowerAddress = deployFlashBorrower(
                uniswapDummyContractAddress
            );
            address flashLoanVaultAddress = deployFlashLoanVault();
            address flashLoanHandlerAddress = deployFlashLoanHandler(
                flashBorrowerAddress,
                flashLoanVaultAddress
            );
        }
    }

    function deployFlashLoanVault()
        public
        payable
        broadcast
        returns (address addr_)
    {
        bytes memory initCode = abi.encodePacked(
            type(FlashLoanVault).creationCode
        );
        addr_ = DeployUtils.deployContract(
            "FlashLoanVault",
            _implSalt(),
            initCode
        );
    ...
    function deployTestUSDToken() public broadcast returns (address addr_) {
        bytes memory initCode = abi.encodePacked(
            type(TestUSDToken).creationCode
        );
        addr_ = DeployUtils.deployContract(
            "TestUSDToken",
            _implSalt(),
    ...
    function deployUniswapDummyContract(
        address testUSDTokenAddress
    ) public payable broadcast returns (address addr_) {
        bytes memory initCode = abi.encodePacked(
            type(UniswapDummyContract).creationCode,
            abi.encode(testUSDTokenAddress)
        );
        addr_ = DeployUtils.deployContract(
            "UniswapDummyContract",
            _implSalt(),
            initCode
        );
    }

    function deployFlashBorrower(
    ...
    function deployFlashLoanHandler(
        address flashBorrowerAddress,
        address flashLoanVaultAddress
    ...
    function _implSalt() internal view returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    vm.envOr("DEPLOY_SALT", string("ethers phoenix"))
                )
            );
    }
}
